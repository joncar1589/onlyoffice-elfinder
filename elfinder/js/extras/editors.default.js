(function(editors, elFinder) {
	...
	return [
		...
		},
		{
			...
			html : `<div id="placeholder" style="width:100%;height:100vh"></div>`,
			// setup on elFinder bootup
			setup : function(opts, fm) {
				if (fm.UA.Mobile || fm.UA.ltIE8 || !fm.onlyofficeURL) {
					this.disabled = true;
				}
			},
			// Prepare on before show dialog
			...
			load : function() {
				...
				let start = function(){
					...
					$this.docEditor = new DocsAPI.DocEditor("placeholder", {
						"document": {
							"fileType": `${types[file.mime]}`,
							"key": `${file.phash}${Math.random()}`,
							"title": `${file.name}`,
							"url": `${file.url}`
						},
						"editorConfig": {
							"callbackUrl": `https://${hostname}/indicadores/editorCallback?source=${hostname}/${file.name}`,
							"lang":"es",
							"mode":"edit",
							"customization": {
								"anonymous": {
								...
								"hideRulers": true,
								/*"logo": {
									"image": "https://test.audimed.app/storage/logos/66e4328aea9dd.png",
									"imageDark": "https://test.audimed.app/storage/logos/66e4328aea9dd.png",
									"url": `${hostname}`,
									"visible": true
								},*/
								"macros": false,
				...
				var self = this, dfrd = $.Deferred();
				if (!self.confObj.loader) {
					self.confObj.loader = $.Deferred();
					if (useRequire) {
						require([
							`${self.fm.onlyofficeURL}web-apps/apps/api/documents/api.js`
						], function(SimpleMDE) {
							self.confObj.loader.resolve(SimpleMDE);
						});
					} else {
						self.fm.loadScript([`${self.fm.onlyofficeURL}web-apps/apps/api/documents/api.js`], function(SimpleMDE) {
							self.confObj.loader.resolve(SimpleMDE);
						}, {loadType: 'tag'});
					}
				}
			...
			// Before dialog close
			close : function(textarea, instance) {
				instance && this.docEditor.requestClose() && this.docEditor.destroyEditor();
